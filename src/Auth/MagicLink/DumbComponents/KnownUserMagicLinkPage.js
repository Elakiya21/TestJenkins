...
class KnownUserMagicLinkPage extends Component {
  state = {
    showErr: false,
    showMsg: "",
  };
  ...
  componentDidMount = () => {
    const { getMagicLinkDetails, dispatch, change, replace, initialize, match, history } =this.props;
    const { id } = match.params;
    dispatch(initialize(registrationForm, {}));
    let param = queryString.parse(location.search);
    let deviceId = param && param.device_id || ""
    getMagicLinkDetails.request({id: id, history: history, deviceId: deviceId})
  };
